...
├── common/          # 横切关注点
│   ├── bootstrap/   # applyAppDefaults：全局前缀 / helmet / ValidationPipe / CORS / Swagger / shutdown 钩子（main.ts 与 e2e 共用）
...
`requestId 中间件 → helmet（非生产关 CSP，Swagger UI 依赖内联脚本）→ CORS（逗号分隔多来源）→ ValidationPipe(whitelist+transform) → RedisThrottlerGuard → JwtAuthGuard → PermissionsGuard → Controller → ResponseEnvelopeInterceptor`
