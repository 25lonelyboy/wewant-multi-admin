...
**目标**：P5 结束时 pure-web 开发模式**直连真实后端**（登录 → 动态路由 → system 三域 CRUD + 详情全走真库），`packages/contracts` 成为前后端契约唯一事实源并建立长期对齐机制；mock 降级为 env 开关的离线模式；文档收尾（architecture 契约规范、ADR-004、AGENTS.md、全局 backlog）。
...
├── auth/                      # 域层：认证
│   └── index.ts               # LoginRequest/TokenPair/登录响应/UserProfile/AsyncRouteNode（动态路由节点）
├── system/                    # 域层：system 三域
...
- **迁移即删除原址**（不留 re-export 过渡壳，typecheck 兜底 import 修正）：
  ...
  - Auth 域响应类型（TokenPair/登录响应/profile/async-routes 节点类型）→ contracts `auth/`；
- **class-validator 的 DTO 类留在 server**（校验是服务端职责），字段形状与契约类型以 `satisfies` 绑定；
- 分页响应统一走 contracts `PageResult<T>`（三域列表端点已是 `{items,total,page,pageSize}` 形态，仅类型来源切换）。
...
- `vite.config.ts`：dev 模式按 `VITE_MOCK` 条件注册 vite-plugin-fake-server（**关闭时整个插件不挂载**，不靠 URL 避让）；关闭时配 proxy `/api/v1` → `http://localhost:<server PORT>`（server `.env` PORT 默认值，实施时核对）；
- `.env.development` 默认 `VITE_MOCK=false`（直连为默认态）；
- mock 文件 URL 全部升级 `/api/v1` 前缀 + 契约同形（信封/字段名/分页/menuType），开关切回 true 时离线全功能可用；两态下前端 api 层代码零差异（同 baseURL、同路径、同形状）；
- mock 响应体以 contracts 类型约束的共享 fixture 组织，压制双源形状漂移。

### 5.2 api 层重写（类型全部来自 contracts）

- `src/api/user.ts`（login/refresh/user-info/async-routes）→ `/api/v1/auth/*`，请求/响应类型换 contracts `auth/`；auth 域路径/方法形态以 P3 已交付端点为准（不做 RESTful 改造，总 spec 既定）；
- `src/api/system.ts` user/role/menu 部分重写为 RESTful：`GET /api/v1/system/users`（PageQuery 走 query 参数，废 POST body 查询）+ POST/PATCH/DELETE + `GET /:id` + 两个分配端点（角色菜单、用户角色），类型换 contracts `system/`；**dept/监控函数原样保留**（决策 #2）；
- 自造的 `Result` / `ResultTable` 类型删除，统一用 contracts `ApiResponse<T>` / `PageResult<T>`。

### 5.3 认证链路适配

- 登录/刷新响应按契约形状适配 `src/utils/auth.ts` `DataInfo`（`expires` 为 ISO 字符串，`new Date()` 解析兼容）；
...
- 分页形态 `{list,total,pageSize,currentPage}` → `PageResult {items,total,page,pageSize}`；
...
适配圈死锁：仅三域 views/hook + api 层 + 拦截器 + store 认证动作，其余模板文件不动。
...
4. access 过期触发 40102 静默刷新（临时调低 `JWT_ACCESS_TTL` 验证），refresh 失效回登录页；
5. 登出后旧 access 被黑名单拒绝；
6. `VITE_MOCK=true` 离线态：登录/路由/三域列表（mock 契约同形）全流程可用；
7. `pnpm build:web` 产物正常；
8. `pnpm check` 全绿 + server `test:coverage` 合并四指标 ≥80% + 双端 typecheck 消费 contracts。
...
- [ ] BizCode/ApiResponse/Auth DTO 类型迁移完成，server 原址删除、import 修正
- [ ] 三域 `GET /:id` 交付（含菜单父链校验），单测 + e2e 覆盖
- [ ] seed 菜单树裁剪（Dept/Monitor），连带测试更新
- [ ] 契约一致性单测每域一条 + e2e 引用契约常量
- [ ] pure-web api 层 RESTful 重写 + 页面适配 + 拦截器 BizCode 化
...
- [ ] `docs/governance/backlog.md` 建立，5 项 backlog 迁移/登记，任务目录归档
