...
B3 共 3 个子任务：组件测试基建沉淀 + **16 个在用组件**全部迁入（含 ReDrawer 移出豁免清单，A3 盘点修正）；8 个遗留组件维持豁免原状，处置留独立决策（总设计第 9 章）。数据来源：A3 盘点落档 + 设计期探查实测。

| 子任务 | 组件 | 规模（总行数，实测） | strict 错误（实测） |
| --- | --- | --- | --- |
| B3.1 | 测试基建 + 低复杂度 7 件：`ReCol` / `ReText` / `ReIcon` / `ReSegmented` / `ReAnimateSelector` / `ReCountTo` / `ReFlicker` | 29+76+4483+244+255+309+44 | 全 0 |
| B3.2 | 中复杂度 6 件：`ReAuth` / `RePerms` / `ReDialog` / `ReTypeit` / `ReImageVerify` / `ReDrawer`（新纳入） | 25+25+560+64+138+499 | Dialog 16 + Drawer 14 |
| B3.3 | 高复杂度 3 件：`RePureTableBar` / `ReCropperPreview` / `ReQrcode` | 474+83+268 | TableBar 16 |
...
- **strict 总负担 46 个错误集中 3 件**（Dialog 16 + TableBar 16 + Drawer 14），其余 13 件零错误
...
| `ReIcon` | `data.ts` 导出完整性断言（导出数、id 结构、去重）全行执行；`Select.vue` 分页 / 筛选 / 复制交互 |
| `ReSegmented` | 内部排序 + 拖拽回调 |
| `ReAnimateSelector` | 动画容器事件透传 |
| `ReCountTo` | 初始 / 目标值 + rebound 配置出入 |
| `ReFlicker` | 乱序内容渲染断言 |
...
| `ReDialog` | 打开 / 关闭状态机 + 16 个 strict 错误机械修复 |
| `ReTypeit` | 实例创建 stub + 生命周期接线 |
| `ReImageVerify` | 输入校验流 + Canvas 绘制豁免（见 6） |
| `ReDrawer` | closeAllDrawer 全局注册表 + 14 个错误修复；**移出豁免清单与迁入清单同一提交** |

### 3.3 高复杂度 3 件

| 组件 | 断言要点 |
| --- | --- |
| `RePureTableBar` | props / 插槽渲染 + sortable 重排回调 + 按钮组回调 + 16 个错误修复 |
| `ReCropperPreview` / `ReQrcode` | 容器渲染 + 关键回调 + Canvas 绘制豁免（见 6） |
...
2. **事实校准窗口**：本设计期数据全部实测（A3 盘点 + 探查：24 组件目录行数、46 个 strict 错误集中 3 件）；计划编写时仅需复核「B1/B2 执行期间是否触碰过 `src/components`」（预期不触碰——两批次域均不含组件层）
