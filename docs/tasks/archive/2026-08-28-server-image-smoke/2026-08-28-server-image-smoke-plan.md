...
- [x] **Step 3: （失败路径自验，仅本地）故意错镜像名验证失败语义**
...
- [x] **Step 4: 暂存（提交合并至 Task 4 后统一执行）**
...
- [x] **Step 1: job 级新增 services（置于 timeout-minutes 与 steps 之间）**
...
- [x] **Step 2: server 构建步骤后新增冒烟 step（置于「构建 nestjs-server 镜像」与「web 镜像启动冒烟」之间）**
...
- [x] **Step 3: 同步 check-digests.sh 计数边界（8 → 10）**
...
- [x] **Step 4: （可选，已存在镜像时跳过）全链路 docker-smoke.sh --server**
...
- [x] **Step 1: build-and-verify.md 更新（两处）**
