...
**结论：影响面集中在新增 `.github/workflows/ci.yml` + ADR-006 + 既有根 README 追加 badge，工程代码零改动。**
...
| 工程代码 | **零改动** | check.mjs / turbo.json / Dockerfile / lockfile 原样复用 |
...
1. **registry 策略**：Dockerfile 硬编码 `registry.npmmirror.com`；GH runner 可达。若 S0 冒烟或首跑实测拉包慢，再提为 `ARG REGISTRY` 注入官方源（不在本轮预改）。
2. **缓存**：v1 只用 `setup-node cache: pnpm`；turbo 远程缓存与 docker 层缓存（`cache-to/from type=gha`）留到时长成痛点再加。
3. **husky 在 CI**：以 `HUSKY: '0'` 显式声明，不依赖第三方行为。
4. **换行符**：Linux runner checkout 得到 LF；Prettier `endOfLine: 'auto'` 不误报。（长期可选：`.gitattributes`。）
5. **secrets 为零**：不发布 = 无 registry 凭据、无部署目标；日后推镜像用内置 `GITHUB_TOKEN` + GHCR。
6. **audit 两段式**：v1 报警式（`continue-on-error` + `--audit-level=high`），S3 复盘噪音基线后再决定是否收紧为失败级。
7. **Docker 验证边界**：web 加启动冒烟；server 依赖 DB 只做 `build` 成功（运行验证由 coverage job 承担）。
8. **成本**：public 仓库 Actions 分钟数无限量；并行墙钟 15~20 分钟。已知冗余：单测在 gate 与 coverage 各跑一次——保持 `pnpm check` 入口一致性优先，接受。
9. **prisma generate 联网**：容器内构建与 runner 上 typecheck/test 均触发引擎二进制下载，GH runner 可达；Dockerfile 已用 `DATABASE_URL` 占位。
...
| 无层缓存时 docker build 慢（单镜像 10~15 分钟） | 接受现状（验证型构建）；痛点后加 `type=gha` 层缓存 |
| npmmirror 在 GH runner 偶发不稳 | 预案：Dockerfile registry 提为 `ARG`，CI 传官方源 |
