...
- 2026-09-05 Task 4 环境观察（非脚本缺陷）：①pnpm install 在 WSL 内无法执行——pnpm.cmd 是 Windows batch 文件，bash 无法解析 `@ECHO` / `GOTO` / `SETLOCAL` 等 batch 指令，属环境限制；②`git check-ignore --version` 在 worktree 内返回非零（WSL 端 git 与 Windows 端 git 路径不一致），降级为仅模式匹配路径正常工作。两项均为 WSL+Windows 混合环境已知约束，不影响脚本正确性。
- 2026-09-05 Task 5 实现期发现（C-7，一项）：`git add -A` 在 worktree 操作后的工作区中会将 699 个文件的 CRLF→LF 行尾规范化变更一并带入提交，污染提交粒度——修复方式：`git reset HEAD~1` 回滚 → `git checkout -- .` 恢复全部文件 → 精确 `git add` 仅 4 个目标文件 → 重新提交（4 files changed, +5 -3）。教训：worktree 操作后的工作区慎用 `git add -A`，应先用 `git status --short` 确认变更范围。
