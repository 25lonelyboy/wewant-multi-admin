...
| `apps/nestjs-server`    | NestJS 后端，阶段二基架补全中：已完成骨架与横切基建、Prisma 7（adapter-pg，client 生成于 `src/generated`）与 Redis（ioredis 自研薄壳）接入、terminus 双探针健康检查、compose 启动链（migrate deploy → seed → node） |
