...
beforeEach(() => {
  vi.clearAllMocks();
  apiMock.getLoginLogsList.mockResolvedValue({
    code: 0,
    data: { list: [], total: 0, pageSize: 10, currentPage: 1 }
  });
});

describe('monitor/logs/login/hook', () => {
  ...
  it('columns 包含序号/用户名/登录IP/登录状态/登录时间', () => {
  ...
  it('onSearch 调用 getLoginLogsList 并更新 dataList', async () => {
    const mockData = {
    ...
    apiMock.getLoginLogsList.mockResolvedValue({ code: 0, data: mockData });
  ...
  it('onSearch 非 0 码时不更新 dataList', async () => {
    apiMock.getLoginLogsList.mockResolvedValue({ code: 1, data: null });
  ...
  it('handleSelectionChange 更新 selectedNum', () => {
    ...
    expect(selectedNum.value).toBe(1);
  });
  ...
  it('loginTime formatter 格式化日期', () => {
    ...
    const result = col.formatter({ loginTime: '2024-03-15T14:20:00' });
