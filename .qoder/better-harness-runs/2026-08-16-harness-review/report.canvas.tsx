...
function practiceDescription(surface) {
  const descriptions = {
    ...
    Memories: [
      'Metadata-only project and global Memory inventory.',
      '仅含元数据的项目级与全局 Memory 盘点。'
    ]
  };
          ...
          <Row gap={5} wrap>
            <Tag size="sm" tone="info">
              {textValue(row.provider) ||
                taskLoopCopy('unknown provider', '未知 provider')}
            </Tag>
            <Tag size="sm" tone="neutral">
              {textValue(row.owner) || '.'}
            </Tag>
...
function usageActivityMatrix(activity, windowIndex = 0) {
  ...
  const maxWindowIndex = Math.floor(
    (latest.getTime() - first.getTime()) / (windowDays * 86_400_000)
  );
  const safeWindowIndex = Math.min(Math.max(0, windowIndex), maxWindowIndex);
  const windowEnd = new Date(
    latest.getTime() - safeWindowIndex * windowDays * 86_400_000
  );
  const windowStart = new Date(
    windowEnd.getTime() - (windowDays - 1) * 86_400_000
  );
  const start = new Date(windowStart);
  start.setUTCDate(start.getUTCDate() - start.getUTCDay());
  const weekCount =
    Math.floor((windowEnd.getTime() - start.getTime()) / (7 * 86_400_000)) + 1;
  const columnGroups = [];
...
function practiceSourceCell(row) {
  const memoryDetails =
    row?.surface === 'Memories' ? memoryPracticeDetails() : null;
  if (memoryDetails) {
    ...
    return (
      <Text size="sm" tone="secondary">
        {scopeSummary ||
          taskLoopCopy('Metadata scan complete', '元数据扫描完成')}
      </Text>
    );
  }
...
function practiceSourceDetail(row) {
  ...
  if (memoryDetails) {
    ...
    return (
      <Stack gap={8} style={{ padding: '8px 4px 4px 16px' }}>
        <Text size="sm" weight="semibold">
          {taskLoopCopy('Memory inventory details', 'Memory 盘点明细')}
        </Text>
        <Row gap={6} wrap>
          {list(memoryDetails.scopeCounts).map(item => (
            <Tag key={item.scope} size="sm" tone="info">
              {item.scope}: {Number(item.count ?? 0)}
            </Tag>
          ))}
          <Tag size="sm" tone="neutral">
            {memoryDetails.scanState}
          </Tag>
        </Row>
        <Text size="sm" tone="secondary">
          {integrityReviewed
            ...
            : taskLoopCopy(
                'Title integrity review unavailable.',
                '标题完整性检查不可用。'
              )}
        </Text>
        <Text size="sm" tone="tertiary">
  ...
  return (
    <SourceLocationsDisclosure
      locations={
        unsuppliedMessage ? [...remaining, unsuppliedMessage] : remaining
      }
      defaultOpen={false}
      label={
        <Text size="sm" weight="semibold">
            ...
            <Button
              ...
              onClick={() =>
                setWindowIndex(value =>
                  Math.min(value + 1, matrix.maxWindowIndex)
                )
              }
            >
    ...
    <Stack gap={7}>
      {rows.map((row, index) => (
        ...
        >
          ...
          <Text size="sm" weight="semibold" style={{ textAlign: 'right' }}>
  ...
  return (
    <Stack gap={12}>
      ...
      <Grid
        columns={2}
        minColumnWidth={260}
        gap={canvasSemanticSpacing.section}
        align="start"
        divided
      >
        <Stack gap={9}>
...
function usageTrendRange(categories) {
  if (!categories.length)
    return taskLoopCopy('Latest observations', '最近观测');
...
function TaskLoopFindingList({ findings }) {
  return (
    <>
      {findings.map(row => (
        <TaskLoopFindingItem key={row.id} row={row} />
      ))}
    </>
  );
}
...
function TaskLoopEvidenceBoundary({ usage }) {
  ...
  return (
    <Stack gap={12}>
      <Callout
        tone={taskLoopMeasurementBoundaryTone(selection, activitySelection)}
      >
...
function TaskLoopReport() {
  ...
  return (
    <ReportShell
      ariaLabel={taskLoopCopy('Better Harness report', 'Better Harness 报告')}
      density="comfortable"
    >
      <Stack gap={canvasSemanticSpacing.reportSection}>
              ...
              <Grid columns={2} minColumnWidth={300} gap={12} align="stretch">
                {repositorySourceRows().map((row, index) => (
                  <RepositorySourceCard
                    key={`${row.provider}-${row.owner}-${row.surface}-${index}`}
                    row={row}
                  />
                ))}
              </Grid>
        ...
        <ReportSection
          title={taskLoopCopy('Evidence and methodology', '证据与方法')}
        >
